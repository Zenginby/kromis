...
| --- | --- | --- | --- | --- |
| GET | `/` | `index`:2132 | `errlog`, `paths`, `version` | — |
| GET | `/api/arena/{arena_id}` | `arena_round_route`:1694 | `paths`, `storage` | `chat.js` |
| POST | `/api/arena/{arena_id}/winner` | `set_arena_winner_route`:1709 | `models`, `paths`, `storage` | `chat.js` |
| GET | `/api/assets/{kind}` | `list_assets_route`:2036 | `assets_store`, `paths` | `assets.js` |
| POST | `/api/assets/{kind}` | `upload_asset`:2014 | `assets_store`, `paths` | `assets.js` |
| DELETE | `/api/assets/{kind}/{asset_id}` | `delete_asset_route`:2048 | `assets_store`, `paths` | `assets.js` |
| POST | `/api/banner` | `add_banner`:1983 | `assets_store`, `models`, `paths`, `storage` | `assets.js` |
| POST | `/api/banner/preview` | `preview_banner`:1975 | `assets_store`, `models`, `paths` | `assets.js` |
| POST | `/api/chat` | `chat`:1332 | `catalog`, `chat_client`, `chat_prompt`, `chat_providers`, `credstore`, `models`, `paths`, `prefs` | `chat.js` |
| DELETE | `/api/chats` | `delete_all_chats_route`:1522 | `chat_store`, `paths` | `chat.js` |
| GET | `/api/chats` | `list_chats_route`:1484 | `chat_store`, `paths` | `chat.js` |
| POST | `/api/chats` | `create_chat_route`:1498 | `chat_store`, `models`, `paths`, `prefs` | `chat.js` |
| DELETE | `/api/chats/{chat_id}` | `delete_chat_route`:1554 | `chat_store`, `paths` | `chat.js` |
| GET | `/api/chats/{chat_id}` | `get_chat_route`:1490 | `chat_store`, `paths` | `chat.js` |
| PUT | `/api/chats/{chat_id}` | `update_chat_route`:1532 | `chat_store`, `models`, `paths`, `prefs` | `chat.js` |
...
| GET | `/api/folders` | `list_folders_route`:1562 | `folders`, `paths`, `storage` | `folders.js` |
| POST | `/api/folders` | `create_folder_route`:1586 | `folders`, `models`, `paths` | `folders.js` |
| DELETE | `/api/folders/{folder_id}` | `delete_folder_route`:1601 | `folders`, `paths`, `storage` | `folders.js` |
| PATCH | `/api/folders/{folder_id}` | `rename_folder_route`:1658 | `folders`, `models`, `paths` | `folders.js` |
| GET | `/api/folders/{folder_id}/download` | `download_folder_route`:1614 | `folders`, `paths` | `folders.js` |
...
| GET | `/api/guncelleme` | `get_guncelleme`:1084 | `guncelleme`, `paths`, `prefs` | `settings.js` |
| GET | `/api/history` | `history`:1672 | `folders`, `paths`, `storage` | `folders.js` |
| DELETE | `/api/image/{image_id}` | `delete_image`:1725 | `paths`, `storage` | `core.js` |
| PATCH | `/api/image/{image_id}` | `move_image`:1684 | `folders`, `models`, `paths`, `storage` | `core.js` |
| DELETE | `/api/images` | `delete_images`:1745 | `models`, `paths`, `storage` | `folders.js` |
| PATCH | `/api/images` | `move_images`:1734 | `folders`, `models`, `paths`, `storage` | `folders.js` |
| POST | `/api/import` | `import_image`:1755 | `folders`, `paths`, `storage` | `folders.js` |
| POST | `/api/logo` | `add_logo`:1908 | `assets_store`, `composite`, `models`, `paths`, `storage` | `assets.js` |
| POST | `/api/logo/preview` | `preview_logo`:1899 | `assets_store`, `composite`, `models`, `paths` | `assets.js` |
| GET | `/api/output/{image_id}/download` | `output_download`:2093 | `paths`, `storage` | `core.js` |
| POST | `/api/palette/suggest` | `suggest_palettes`:1796 | `color_names`, `models`, `palette` | `palette.js` |
| GET | `/api/palettes` | `list_palettes_route`:1827 | `palette_store`, `paths` | `palette.js` |
| POST | `/api/palettes` | `create_palette_route`:1832 | `color_names`, `models`, `palette`, `palette_store`, `paths` | `palette.js` |
| DELETE | `/api/palettes/{palette_id}` | `delete_palette_route`:1853 | `palette_store`, `paths` | `palette.js` |
| GET | `/api/prefs` | `get_prefs_route`:1407 | `paths`, `prefs` | `chat.js`, `core.js`, `settings.js` |
| POST | `/api/prefs` | `post_prefs_route`:1413 | `models`, `paths`, `prefs` | `chat.js`, `core.js`, `settings.js` |
| GET | `/api/settings` | `get_settings`:1049 | `azure_client`, `catalog`, `credstore`, `guncelleme`, `paths`, `prefs`, `version` | `settings.js` |
| POST | `/api/settings` | `post_settings`:1113 | `azure_client`, `catalog`, `credstore`, `models`, `version` | `settings.js` |
...
| GET | `/assets/{kind}/{filename}` | `asset_file`:2057 | `assets_store`, `paths` | `assets.js` |
| GET | `/output/{filename}` | `output_file`:2069 | `paths`, `storage` | `assets.js`, `chat.js`, `core.js`, `folders.js`, `viewer.js` |
