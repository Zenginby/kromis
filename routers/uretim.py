...
# anahtar id'yi taşır. Rezerv EN SONDA ve satırdan sonra (K2/K9): bakiyeyi
# DEĞİŞTİREN tek kapı, tavan yalnız okur — "ucuz ve salt okunur önce, yazan
# en sonda"; rezerv satırı `is_id` ister (FK), o yüzden `ekle`den sonra;
# yetmezse rota 402 fırlatır, `OTURUM` transaksiyonu geri alır — iş satırı da
# gider, kuyrukta öksüz iş yok, bakiye dokunulmamış. BYOK iş (`kullanici`)
# defteri hiç görmez (K3). Ön denetim gerçek kapı DEĞİL: iki istek aynı
# bakiyeye yarışırsa kararı `rezerve`nin `WHERE bakiye >= :m`i verir.
#
...
def _siraya_koy(db: Session, kullanici: Kullanici, tur: str, istek: dict[str, Any],
                model: str, kredi_tahmini: int, is_id: uuid.UUID | None = None,
                anahtar_kaynagi: str | None = None) -> dict:
    """`kuyruk.ekle` → rezerv (platform işi; 402) → 202 gövdesi. `istek` DÖKÜLMEZ (`_json`): prompt ve girdi anahtarları içeride.

    Satır ve rezerv AYNI transaksiyonda (sıra dosya başında): `ekle` flush
    eder, `rezerve_kredi` bakiyeyi atomik düşer ve `rezerv:<is_id>` satırını
    yazar; yetmezse fırlayan 402 ikisini de geri aldırır.
    """
    is_ = kuyruk.ekle(db, kullanici.id, tur, istek, model, kredi_tahmini, is_id=is_id,
                      anahtar_kaynagi=anahtar_kaynagi)
    kapilar.rezerve_kredi(db, kullanici, is_.id, kredi_tahmini, anahtar_kaynagi)
...
def _kapilar(db: Session, kullanici: Kullanici, cred_id: str, kimlikler: Mapping[str, str],
    ...
    kapilar.check_bakiye(db, kullanici, kredi_tahmini, kaynak)
    return kaynak
...
def generate(req: GenerateRequest, db: Session = OTURUM,
    ...
    return _siraya_koy(db, kullanici, "generate", istek, req.model, kredi_tahmini,
...
async def animate(
    ...
    return await run_in_threadpool(_siraya_koy, db, kullanici, "animate", istek, model_id,
...
async def edit(
    ...
    return await run_in_threadpool(_siraya_koy, db, kullanici, "edit", istek, model_id,
                                   kredi_tahmini, is_id, kaynak)
