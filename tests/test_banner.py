...
def _setup(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici):
    ...
    src_id = uret_ve_bitir(c, "/api/generate", json={"prompt": "cat", "size": "1024x1024",
...
def test_banner_preview_returns_data_url_and_does_not_save(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, src_id, banner_id = _setup(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
...
def test_banner_apply_creates_derivative(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, src_id, banner_id = _setup(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
...
def test_banner_rejects_bad_edge(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, src_id, banner_id = _setup(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
    r = c.post("/api/banner", json={"id": src_id, "asset_id": banner_id, "edge": "middle"})
    assert r.status_code == 422


def test_banner_404_for_unknown_banner(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, src_id, _ = _setup(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
    r = c.post("/api/banner", json={"id": src_id, "asset_id": "deadbeef01"})
    assert r.status_code == 404


def test_banner_404_for_unknown_source(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, _, banner_id = _setup(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
...
def _setup_colored(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici):
    ...
    c = TestClient(appmod.app)
    src_id = uret_ve_bitir(c, "/api/generate", json={"prompt": "x", "size": "1024x1024",
...
def test_banner_scale_and_right_align_places_strip_on_the_right(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, src_id, banner_id = _setup_colored(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
...
def test_banner_left_align_places_strip_on_the_left(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, src_id, banner_id = _setup_colored(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
    im = _preview_image(c, {"id": src_id, "asset_id": banner_id, "edge": "bottom",
...
def test_banner_margin_pushes_strip_away_from_edge(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, src_id, banner_id = _setup_colored(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
    im = _preview_image(c, {"id": src_id, "asset_id": banner_id, "edge": "bottom",
...
def test_banner_defaults_match_full_width_behaviour(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
...
def test_banner_rejects_bad_align(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, src_id, banner_id = _setup_colored(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
    r = c.post("/api/banner", json={"id": src_id, "asset_id": banner_id, "align": "diagonal"})
    assert r.status_code == 422


def test_banner_rejects_unknown_field_instead_of_silently_ignoring(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
...
def test_logo_rejects_unknown_field(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, src_id, _ = _setup_colored(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
    r = c.post("/api/logo/preview", json={"id": src_id, "gelecekteki_secenek": 1})
    assert r.status_code == 422


def test_banner_rejects_out_of_range_scale_and_margin(tmp_path, monkeypatch, dizinler, db_oturumu, kullanici, uret_ve_bitir):
    c, src_id, banner_id = _setup_colored(uret_ve_bitir, tmp_path, monkeypatch, dizinler, db_oturumu, kullanici)
