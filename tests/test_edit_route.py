...
def test_edit_from_upload(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
...
def test_edit_from_source_id(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
    ...
    r = uret_ve_bitir(c, "/api/edit", data={"prompt": "make blue", "size": "1024x1024",
                                  "quality": "low", "n": "1", "source_id": src_id})
    assert r.status_code == 200, r.text
    assert r.json()["images"][0]["parent_id"] == src_id


def test_edit_labels_the_image_with_its_session(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
...
def test_edit_without_a_session_writes_no_session_key(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
    monkeypatch.setattr(ac, "edit", lambda *a, **k: [b"\x89PNG-edited"])
    c = _client(tmp_path, dizinler)
...
def test_edit_rejects_a_malformed_session_id(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
    monkeypatch.setattr(ac, "edit", lambda *a, **k: [b"\x89PNG-edited"])
    c = _client(tmp_path, dizinler)
...
def test_edit_rejects_both_file_and_source_id(tmp_path, dizinler, uret_ve_bitir):
    c = _client(tmp_path, dizinler)
    r = uret_ve_bitir(c, "/api/edit",
...
def test_edit_rejects_neither(tmp_path, dizinler, uret_ve_bitir):
    c = _client(tmp_path, dizinler)
    r = uret_ve_bitir(c, "/api/edit", data={"prompt": "x", "size": "1024x1024",
                                  "quality": "low", "n": "1"})
    assert r.status_code == 422


def test_edit_rejects_bad_size(tmp_path, dizinler, uret_ve_bitir):
    c = _client(tmp_path, dizinler)
    r = uret_ve_bitir(c, "/api/edit",
...
def test_edit_rejects_invalid_upload(tmp_path, dizinler, uret_ve_bitir):
    c = _client(tmp_path, dizinler)
    r = uret_ve_bitir(c, "/api/edit",
...
def test_edit_rejects_oversized_upload(tmp_path, dizinler, uret_ve_bitir):
    c = _client(tmp_path, dizinler)
...
def test_edit_unknown_source_404(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
    c = _client(tmp_path, dizinler)
    r = uret_ve_bitir(c, "/api/edit", data={"prompt": "x", "size": "1024x1024",
...
def test_edit_rejects_huge_dimensions(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
...
def test_edit_with_extra_upload_sends_both_images_in_order(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
...
def test_edit_with_extra_source_id_uses_gallery_image(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
    monkeypatch.setattr(ac, "generate", lambda *a, **k: [_png_bytes()])
    c = _client(tmp_path, dizinler)
    ids = [
        uret_ve_bitir(c, "/api/generate", json={"prompt": p, "size": "1024x1024",
                                      "quality": "low", "n": 1}).json()["images"][0]["id"]
        for p in ("cat", "dog")
    ]
    seen = _capture_edit(monkeypatch)
    r = uret_ve_bitir(c, "/api/edit", data={"prompt": "birleştir", "size": "1024x1024", "quality": "low",
...
def test_edit_rejects_more_than_max_images(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
...
def test_edit_rejects_invalid_extra_upload(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
    _capture_edit(monkeypatch)
    c = _client(tmp_path, dizinler)
    r = uret_ve_bitir(c, "/api/edit",
        data={"prompt": "x", "size": "1024x1024", "quality": "low", "n": "1"},
...
def test_edit_unknown_extra_source_404(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
    _capture_edit(monkeypatch)
    c = _client(tmp_path, dizinler)
    r = uret_ve_bitir(c, "/api/edit",
        data={"prompt": "x", "size": "1024x1024", "quality": "low", "n": "1",
...
def test_edit_ignores_empty_extra_fields(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
...
def test_edit_maps_azure_error(tmp_path, monkeypatch, dizinler, uret_ve_bitir):
    def boom(*a, **k):
