...
# ── nereye düşüyor ──────────────────────────────────────────────────────
def test_import_lands_in_the_root_when_no_folder_given(tmp_path, monkeypatch, dizinler):
...
def test_import_files_the_image_into_the_named_folder(tmp_path, monkeypatch, dizinler):
...
def test_import_with_unknown_folder_404(tmp_path, monkeypatch, dizinler):
...
# ── dosyanın kendisi ────────────────────────────────────────────────────
def test_import_reencodes_a_jpeg_as_png(tmp_path, monkeypatch, dizinler):
...
def test_import_rejects_a_non_image_file(tmp_path, monkeypatch, dizinler):
    c = _client(tmp_path, monkeypatch, dizinler)
...
def test_import_rejects_an_oversized_file(tmp_path, monkeypatch, dizinler):
...
def test_import_records_the_real_pixel_dimensions(tmp_path, monkeypatch, dizinler):
...
def test_import_label_is_the_bare_filename(tmp_path, monkeypatch, dizinler):
...
def test_import_truncates_a_very_long_filename(tmp_path, monkeypatch, dizinler):
    c = _client(tmp_path, monkeypatch, dizinler)
    rec = _import(c, filename="a" * 300 + ".png").json()["image"]
    assert len(rec["prompt"]) == 120


def test_import_falls_back_to_a_label_when_the_filename_is_empty(tmp_path, monkeypatch, dizinler):
...
def test_imported_record_has_no_parent_and_no_palette(tmp_path, monkeypatch, dizinler):
...
def test_import_marks_the_record_as_imported(tmp_path, monkeypatch, dizinler):
    """Galeri işareti buna bakıyor; düşerse içe aktarılan üretilmiş gibi görünür."""
    c = _client(tmp_path, monkeypatch, dizinler)
    assert _import(c).json()["image"]["imported"] is True


def test_two_imports_both_survive_in_history(tmp_path, monkeypatch, dizinler):
...
def test_ice_aktarilan_gorsel_bir_MODEL_iddia_ETMIYOR(tmp_path, monkeypatch, dizinler):
    ...
    """
    r = _import(_client(tmp_path, monkeypatch, dizinler))
