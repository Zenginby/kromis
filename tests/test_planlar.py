...
# mandallı olsun: bir gün Kling V3 Pro "free"ye düşerse (ya da schnell
# "temel"e çıkarsa) burası kırmızı olur. GPT Image 2.5 ilk sürümde `temel`
# örneğiydi; sahibin fiyat araştırması (PR #80 yorumu) medium'u 3 kredi
# bulunca `free`ye indi — görselde artık basamaklı girdi YOK, basamak
# örnekleri video tarafından.
...
@pytest.mark.parametrize("spec, plan, kaynak, beklenen, sebep", [
    # Ücretsiz + platform anahtarı: 1 kredilik schnell ve 3 kredilik GPT Image 2.5 AÇIK (ikisi de free).
    (SCHNELL, "free", PLATFORM, True, None),
    (GPT25, "free", PLATFORM, True, None),
    (GPT25, "free", BENIM, True, None),
    (GPT25, "temel", PLATFORM, True, None),
    # 1b-A: kendi fal anahtarı GÖRSEL eşiğini aşardı — ama video eşiğini (K7) aşmaz; Kling V3 Pro
    # `temel` ve video, ücretsiz kullanıcı iki kaynakla da görmez.
    (KLING_PRO, "free", PLATFORM, False, "plan"),
    (KLING_PRO, "free", BENIM, False, "plan"),
...
def test_check_plan_agrees_with_model_available_on_the_real_gpt_image_2_5(depo_db, kullanici):
    ...
    ücretsiz kullanıcı `temel` basamaklı Kling V3 Pro'ya iki anahtar kaynağıyla da 403 alır
    (video, K7); GPT Image 2.5 (free, 3 kredi) ve schnell her iki kaynakla geçer."""
    assert GPT25 is not None and SCHNELL is not None and KLING_PRO is not None
    with Session(depo_db) as s:
        for kaynak in (PLATFORM, BENIM):
            with pytest.raises(HTTPException) as e:
                kapilar.check_plan(s, kullanici, KLING_PRO, _kimlikler(KLING_PRO, kaynak))
            assert e.value.status_code == 403 and e.value.detail["model"] == KLING_PRO.id
        assert kapilar.check_plan(s, kullanici, GPT25, _kimlikler(GPT25, PLATFORM)) == "free"
